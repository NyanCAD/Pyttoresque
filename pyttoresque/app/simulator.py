...
class TranSimulation(Simulation):
    ...
    def cmd(self, fs):
        return fs.commands.tran(self.maximum_timpestep, self.stop_time, self.start_time, self.vectors)
...
class OpSimulation(Simulation):
    # @param.depends('vectors', watch=True, on_init=True)
    def cmd(self, fs):
...
class Simulator(param.Parameterized):
    ...
    def _cmd(self):
        ...
        self.res.cmd = lambda _: self.tabs.sim.cmd(self.conf.connect()).wait()
